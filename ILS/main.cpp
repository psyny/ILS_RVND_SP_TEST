...
bool runInstance(commandline& c, Params& params, std::string inputFile, std::string outputFile) {
	...
	int timeLimitSecs = ((int)((double)2.4 * params.nbClients));
	params.timeLimit = timeLimitSecs * CLOCKS_PER_SEC; // 240 seconds for each 100 clients, or 2.4 seconds for client
	std::cout << "----- START OF THE ALGORITHM -----" << std::endl;
	std::cout << "----- time limit: " << timeLimitSecs << "s -----" << std::endl;
...
int main(int argc, char *argv[])
{
	...
	if (c.is_valid())
	{
		if (c.is_all() || c.is_allX() || c.is_allA() || c.is_allC() || c.is_allG()) {
			std::cout << "--- BATCH RUN INSTANCES ---" << std::endl;
			std::cout << "------ types: ";
			// Runs all registered instance of a defined type (or all all)
			// All file names to be considered
			std::vector<std::list<std::string>> allFiles = std::vector<std::list<std::string>>();

			// Instances A
			if (c.is_all() || c.is_allA())
			{
				std::cout << "A ";
			...
			// Instances C
			if (c.is_all() || c.is_allC())
			{
				std::cout << "C ";
			}

			// Instances G
			if (c.is_all() || c.is_allG())
			{
				std::cout << "G ";
			}

			// Instances X
			if (c.is_all() || c.is_allX())
			{
				std::cout << "X ";
				std::list<std::string> files({
			...
			// Run all registered instances
			std::string instancesFolder = "../Instances/";
			
			for (int instType = 0; instType < allFiles.size(); instType++)
			{
				std::list<std::string> filesOfInstance = allFiles[instType];
				for (std::string fileName : filesOfInstance) {
